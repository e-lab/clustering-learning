...
if opt.visualize then
   f256S = data[{{1,256}}]:reshape(256,3,is,is)[{{},{1},{},{}}]:reshape(256,is,is)
...
--if not paths.filep('cifar10-1l.t7') then
   ...
   function cb (kernels)
      if opt.visualize then
         win = image.display{image=kernels:reshape(nk,3,is,is)[{{},{1},{},{}}]:reshape(nk,is,is), padding=2, symmetric=true, zoom=2, win=win, nrow=math.floor(math.sqrt(nk)), legend='1st layer filters'}
      end
   end                    
...
l1net.modules[4].weight = torch.ones(1)*(1/is)*(1/is)
