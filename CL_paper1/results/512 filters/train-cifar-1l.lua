...
l1net = nn.Sequential()
l1net:add(nn.SpatialConvolution(3, nk, is, is))
l1net:add(nn.Tanh())
...
for i=1,nk do   
   l1net.modules[1].weight[i] = kernels[i]:reshape(3, is, is):type('torch.DoubleTensor')
end
l1net.modules[1].bias = l1net.modules[1].bias *0

-- processing dataset:
trainData2 = {
   data = torch.Tensor(trsize, nk*(o1size/poolsize)^2),
   labels = trainData.labels:clone(),
   size = function() return trsize end
}
testData2 = {
   data = torch.Tensor(tesize, nk*(o1size/poolsize)^2),
...
trainData2.data = trainData2.data:reshape(trsize,nk,o1size/poolsize,o1size/poolsize)
testData2.data = testData2.data:reshape(tesize,nk,o1size/poolsize,o1size/poolsize)
...
model = nn.Sequential()
model:add(nn.Reshape(nk*(o1size/poolsize)^2))
model:add(nn.Linear(nk*(o1size/poolsize)^2,nhiddens))
model:add(nn.Tanh())
