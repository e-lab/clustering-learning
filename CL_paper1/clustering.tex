...
\begin{enumerate}
\item SpatialSAD module: performing sum-abs-diff operation on images convolutionally with the learned CL filters: $yc_i=\sum_i {k_{ij} - x_i}$.
...
We tested random filters used on the 2nd layer of the networks  for both SVHN and CIFAR10 datasets. The 1st layer of the network was trained with CL, as mentioned above. The randomly connected 2nd layer used a fixed CNN layer as described in section \ref{sec-net-arch}. The SpatialConvolution module was initialized with uniformly random filters by the Torch7 tool, and left untouched. % needs better explanation? untouched -- in contrast to being updated with the backprop of the last two layers
