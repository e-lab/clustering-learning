...
--w1=image.display(inputim)
--w2=image.display(Ninputim)

-- kernel/patch size
fil_s = 31 -- has to be odd sized
fil2 = (fil_s+1)/2 -- because of this...
...
--print(inputim:size())
ker=torch.zeros(inputim:size(1), fil_s, fil_s)
ker=image.crop(Ninputim, px, py, px+fil_s, py+fil_s)
...
function process()
	...
	processed=temp1+temp2+temp3
	processed=processed/torch.max(processed)
	processed = processed:mul(-1):add(1)
	--print(torch.min(processed), torch.max(processed))

	outim:sub(1,1, 1,our, 1,ouc):copy(processed)
	outim:sub(2,2, 1,our, 1,ouc):copy(processed)
	outim:sub(3,3, 1,our, 1,ouc):copy(processed)
	
	-- update object/patch location
	value, px_nxt, py_nxt = GetMax(outim[2])
	print('SAD output:', value)
	--:sub(
--		math.max(py-src_rng,1),
--		math.min(py+src_rng,our),
--		math.max(px-src_rng,1), 
--		math.min(px+src_rng,ouc)))
...
function display()
   ...
   win:setcolor(1,0,0)
   win:rectangle(px-fil2, py-fil2, fil_s, fil_s)
   win:stroke()
