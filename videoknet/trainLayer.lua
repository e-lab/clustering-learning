...
function createCovCnxTable(vdata, fanin)
   -- create a covariance connection table based on some test data:
   
   nf = vdata:size(1) --number frames
   nk = vdata:size(2) -- number features
   sz_X = vdata:size(3) -- image size
   sz_Y = vdata:size(4)
   
   covMat = torch.zeros(nk,nk) --covariance matrix
   connTable = {}
   ...
   for i=1,nk do
      max, j = torch.sort(covMat[i])
      for k=1,fanin do -- skip first value because it would connect to itself
