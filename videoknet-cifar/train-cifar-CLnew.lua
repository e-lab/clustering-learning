...
if opt.slacmodel then
   ...
   kernels1 = trainLayer(nlayer, trainData.data, opt.nsamples, nil, nk1, nnf1, is) 
end

image.display{image=kernels1:reshape(kernels1:size(1),is,is), padding=2, symmetric=true, zoom=4, legend = 'Layer 1 filters'}
...
kernels1_ = kernels1:clone():div(nnf1*nk1/2) -- divide kernels so output of SpatialConv is about ~1 or more
if opt.slacmodel then vnet.modules[1].weight = kernels1s_:reshape(nk1s,is,is)
      else vnet.modules[1].weight = kernels1_:reshape(nk1,is,is) end

----------------------------------------------------------------------
print '==> process dataset throught 1st layer:'
trainData2 = processLayer(nlayer, vnet, trainData.data, 1, ovhe, ovwi)
testData2 = processLayer(nlayer, vnet, testData.data, 1, ovhe, ovwi)
...
-- test of max output feature space:
kernels2 = trainLayer(nlayer, trainData2, opt.nsamples, nil, nk2, nnf2, is) 

image.display{image=kernels2:reshape(kernels2:size(1),is,is), padding=2, symmetric=true, zoom=4, legend = 'Layer 2 filters'}
...
kernels2_= kernels2:clone():div(nk2/4) -- divide kernels so output of SpatialConv is about ~1 or more
--vnet2.modules[1].weight = kernels2:reshape(nk2,nk1,is,is) --full connex filters
vnet2.modules[1].weight = kernels2_:reshape(nk2,1,is,is)  -- OR-AND model *3/2 because of fanin and 2*fanin connnex table
