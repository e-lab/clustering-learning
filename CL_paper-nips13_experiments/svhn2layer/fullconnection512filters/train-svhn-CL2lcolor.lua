...
poolsize = 2
cvstepsize = 1
if opt.whitening then cvstepsize = 5 end
...
if opt.whitening then
   ...
   step1 = 1000
   trsize = math.floor(trsize/step1)*step1
   tesize = math.floor(tesize/step1)*step1
   trainData2 = torch.Tensor(trsize, nk1, ovhe, ovhe)
   testData2 = torch.Tensor(tesize, nk1, ovhe, ovhe)

                                                                                                               
  for i=1, trsize/step1 do                                                                                                         
...
feat_group = 8 --features per group (32=best in CIFAR, nk1=32, fanin=2)
poolsize = 2
cvstepsize = 1
...
kernels2, counts1, M, P = trainLayer(nlayer, trainData2, opt.nsamples, nil, nk2, nnf2, is)

vnet2 = nn.Sequential()
vnet2:add(nn.SpatialConvolution(nk1, nk2,  is, is, cvstepsize,cvstepsize)) -- connex table based on 
...
vnet2.modules[1].weight = kernels2:reshape(nk2, nk1,is,is)  
...
std= {}
for i=1, nk2 do 
 mean[i] = trainData3[{ {},i,{},{} }]:mean()
 std[i] = trainData3[{ {},i,{},{} }]:std()
 trainData3[{ {},i,{},{} }]:add(-mean[i])
 trainData3[{ {},i,{},{} }]:div(std[i])
 testData3[{ {},i,{},{} }]:add(-mean[i])
 testData3[{ {},i,{},{} }]:div(std[i])
end

print('trdata3', trainData3[1])
l1netoutsize = ovhe2
...
for t = 1,trsize do
   trainDataF.data[t] = torch.cat(trainData3[t]:reshape(nk*(l1netoutsize)^2), trainDataF.color[t]:reshape(3*(cdatasize)^2))
   xlua.progress(t, trainData:size())
end
for t = 1,tesize do
   testDataF.data[t] = torch.cat(testData3[t]:reshape(nk*(l1netoutsize)^2), testDataF.color[t]:reshape(3*(cdatasize)^2))
   xlua.progress(t, testData:size())
end
