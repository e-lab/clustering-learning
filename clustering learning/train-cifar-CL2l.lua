...
cmd:option('-initstd', 0.025, 'standard deviation to generate random initial templates')
...
trainData = torch.load('trainData-xxx.t7')
testData = torch.load('testData-xxx.t7')

-- show a few outputs:
--image.display{image=trainData.data[{{1,256},{1}}]:reshape(256,12,12),padding=2, symmetric=true, zoom=2}
...
--kernels = torch.load('cifar10-2l-256c.t7')
