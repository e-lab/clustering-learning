...
----------------------------------------------------------------------
print '==> loading pre-processed dataset with 1st layer clustering (test-SVHN-1l.lua)'
...
image.display{image=trainData.data[{{1,256},{1}}]:reshape(256,6,6),padding=2, symmetric=true, zoom=2}
...
image.display{image=data[{{1,256}}]:reshape(256,is,is),
               padding=2, symmetric=true, zoom=2,
               nrow=math.floor(math.sqrt(nk)),
               legend='image patches'}
...
torch.save('SVHN-2l.t7', {kernels, kcounts})
...
kernels = torch.load('crapSVHN-2l-256.t7')
...
--
--l1net = nn.Sequential()
--l1net:add(nn.SpatialConvolution(nk, nk, is, is, cvstepsize, cvstepsize))
--l1net:add(nn.Tanh())
...
--l1net.modules[1].weight = kernels:reshape(nk, 1, is, is):expand(nk,nk,is,is)
