...
The goal of this paper was to provide a simple and fast method to train unsupervised networks for general-purposed robotic vision system. We decided to test our networks in a tracking task, where the network has to be able to track an object of interest based on a  single presentation. For this purpose we use the challenging benchmark TLD dataset \cite{Kalal2011}. From this dataset we selected multiple videos with different properties of occlusions, camera movement, pose, scale and illumination changes.

We use the same two-layer network architecture used in \cite{Farabet2012} for insuring real-time operation of ~6 frames/s on a quad-code Intel i7 laptop computer. We focused on this network and restricted ourselves to real-time operation because the goal of this project is the use of deep networks in mobile computers. The network operates on 46 x 46 input images, uses 16 7 x 7 filters on the first layer and 128 7 x 7 filters on the second layer. The 1st to 2nd layer fan in was 8, and the 2nd to 3rd layer fanin was 64. Both layers were connected with random tables. The network produces a 128 feature vector as output. We trained the same size and number of filters through clustering algorithm for use in this network. We used patches from a contrast normalized version of a few images from the Berkeley image dataset \cite{MartinFTM01}. Any group of natural-scene images can be used to train the general-purpose network presented in this section, and we on purpose chose not to sample patches from the target TLD dataset, in order to demonstrate the learning invariance property of our techniques.
...
\caption{Test set accuracy comparison for a convolutional neural network and a Clustering Learning network with 1 and 2 layers on the CIFAR10 dataset}
\label{data_cifar}
\end{center}
\end{figure}
...
As can be seen in Table \ref{table1}, the Clustering Learning (CL) network performs similarly to the convolutional neural network from \cite{Farabet2012} in sequence "Jumping", "Pedestrian 2 and 3", "Pedestrian 3", it also performs better in sequence "Pedestrian 1" and worse in sequence "Car".
...
\begin{table}[htdp]
\caption{Precision comparison between of a Clustering Learning (CL) network  and a convolutional neural network  \cite{Farabet2012} used as trackers in the TLD dataset}
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline\hline
Sequence 	& Frames 	& CL Network precision	& Network from \cite{Farabet2012}  precision \\ 
\hline
Jumping 	& 313 	& 0.18 	& \textbf{0.20} \\ 
Pedestrian 1 	& 140 	& \textbf{0.811} 	& 0.69 \\ 
Pedestrian 2 	& 338 	& 0.78 	& \textbf{0.79} \\ 
Pedestrian 3 	& 184 	& 0.35  	& \textbf{0.44} \\ 
Car 	& 945 	& 0.25  	& \textbf{0.48} \\ 
\hline\hline
\end{tabular}
\end{center}
\label{table1}
