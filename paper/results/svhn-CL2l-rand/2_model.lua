...
elseif opt.model == '1st-layer-dist' then   
   ...
   model:add(nn.Reshape(nk1,o1size,o1size))
   --model:add(nn.Abs()) model:add(nn.HardTanh()) -- does not help: -1% cifar 1layer
   model:add(nn.Tanh())
   --model:add(nn.SpatialSubSampling(nk1, poolsize, poolsize, poolsize, poolsize)) -- bad results here
   model:add(nn.SpatialLPPooling(nk1, 2, poolsize, poolsize, poolsize, poolsize)) 
   model:add(nn.SpatialSubtractiveNormalization(nk1, normkernel))
   
elseif opt.model == '1st-layer-dist-cn' then  
...
elseif opt.model == '2nd-layer-dmap' then   
