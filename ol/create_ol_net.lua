...
-- load pre-trained SAD modules for each layer:
sad1 = torch.load('berkeley-sadmap1.net')
sad2 = torch.load('berkeley-sadmap2.net')
sad3 = torch.load('berkeley-sadmap3.net')
...
if opt.archn == 1 then
   ...
   -- 2nd layer:
   --net_new:add(nn.SpatialSADMap(nn.tables.random(nk1, nk2, fanin2), is2, is2))
   net_new:add(sad2)
   ...
   --3rd layer:
   --net_new:add(nn.SpatialSADMap(nn.tables.random(nk2, nk3, fanin3), is3, is3))
   net_new:add(sad3)
...
torch.save('encoder_new2.net', net_new)
