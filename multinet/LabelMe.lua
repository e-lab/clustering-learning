--------------------------------------------------------------------------------
-- LabelMe: A class to handle datasets from LabelMe (and other segmentation
--                 based datasets).
--
...
function LabelMe:__init(...)
   ...
   xlua.unpack_class(
      ...
      {arg='classToSkip', type='table', help='index of class to skip during sampling', default=0},
   ...
   -- parse dir structure
   print('<LabelMe> loading LabelMe dataset from '..self.path)
   ...
   if self.samplingMode == 'equal' or self.samplingMode == 'random' then
      ...
      if self.samplingMode == 'random' then
         ...
         for i,v in ipairs(self.tags) do
            local classSkip = 1
            for l, m in pairs(self.classToSkip) do
               if(m==i) then classSkip=0 end
            end
            if classSkip then
         ...
         for i,v in ipairs(self.tags) do
            ...
            if classSkip and v.size > 0 then
      ...
      error('ERROR <LabelMe> unknown sampling mode')
   end
...
function LabelMe:getsizes(folder,file)
   ...
   elseif file:find('.mat$') then
      if not xrequire 'mattorch' then 
         xerror('<LabelMe> mattorch package required to handle MAT files')
      end
      ...
      xerror('images must either be JPG, PNG or MAT files', 'LabelMe')
   end
...
function LabelMe:size()
   return self.nbSamples
end

function LabelMe:__tostring__()
   local str = 'LabelMe:\n'
...
function LabelMe:__index__(key)
   ...
   if type(key)=='number' then
      ...
      elseif self.samplingMode == 'equal' then
         ...
         classSkip = 0
         for k, m in pairs(self.classToSkip) do
            if(self.classToSkip[k]==ctr_target) then classSkip=1 end
         end
         while self.tags[ctr_target].size == 0 or classSkip == 1 do
...
function LabelMe:loadSample(index)
...
function LabelMe:preload(saveFile)
   ...
   print('<LabelMe> preloading all images')
   ...
   if self.cacheFile then
      print('<LabelMe> saving samples to cache file: '
      .. paths.concat(self.path,self.cacheFile..'-samples'))
...
function LabelMe:parseMask(existing_tags)
   local tags
...
function LabelMe:parseAllMasks(saveFile)
   ...
   if self.cacheFile and paths.filep(paths.concat(self.path,self.cacheFile..'-tags')) then
      print('<LabelMe> retrieving saved tags from :' .. paths.concat(self.path,self.cacheFile..'-tags')
      .. ' [delete file to force new scan]')
   ...
   print('<LabelMe> parsing all masks to generate list of tags')
   print('<LabelMe> WARNING: this operation could allocate up to '..
   ...
   -- report
   print('<LabelMe> nb of patches extracted per category:')
   for i = 1,self.nbClasses do
   ...
   if self.cacheFile then
      print('<LabelMe> saving tags to cache file: ' .. paths.concat(self.path,self.cacheFile..'-tags'))
...
function LabelMe:display(...)
   -- check args
   local _, title, samples, zoom = xlua.unpack(
   {...},
   'LabelMe.display',
   'display masks, overlayed on dataset images',
   {arg='title', type='string', help='window title', default='LabelMe'},
